...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр) Экспорт
	...
	Для каждого Тест Из ОписаниеТестов Цикл
		Если ТекущаяГруппа <> Тест.ТипОснование Тогда
			...
			НаборТестов.НачатьГруппу("Тип документа основания """ + ТипЗнч(Тест.Основание) + """", Ложь);// add_plus
		КонецЕсли;
		
		// add_plus
		ИмяНастройки = ""+Тест.ВводитсяНаОсновании.Имя+"/"+Тест.ТипОснование;
		
		Если Тест.Проведен Тогда
			Рез = СписокИсключений_ДокументыПроведенные.Найти(ИмяНастройки);
		Иначе
			Рез = СписокИсключений_ДокументыНеПроведенные.Найти(ИмяНастройки);
		КонецЕсли;
		...
		НаборТестов.Добавить("ТестДолжен_ВыполнитьОткрытиеФормыДокументаНаОснованииДругогоДокумента",
							  ...
							  ТипЗнч(Тест.Основание) + """ Настройка: " + ИмяНастройки);		  
		ЕстьТесты = Истина; 
		// add_plus
	КонецЦикла;
