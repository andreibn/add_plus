...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр, НастройкиВходящие = Неопределено) Экспорт // add_plus
	...
	Для Каждого МетаОбъект Из Метаданные.Документы Цикл
		Если МетаОбъект.Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить 
			...
			ДобавитьТестыДляДокумента(НаборТестов, МетаОбъект, ЕстьТесты);
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ЗагрузитьНастройки(НастройкиВходящие) // add_plus
	...
	Если НастройкиВходящие = Неопределено Тогда
		ПлагинНастроек = КонтекстЯдра.Плагин("Настройки");
		Настройки = ПлагинНастроек.ПолучитьНастройку(ИмяТеста());
	КонецЕсли;
...
Процедура ДобавитьТестыДляДокумента(НаборТестов, МетаОбъект, ЕстьТесты) // add_plus
	...
	|	Проведен //{Отбор}
	// add_plus
	|	И Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДата, МЕСЯЦ, -6) 
	|	И Дата <= &ТекущаяДата
	// add_plus
	|
	|УПОРЯДОЧИТЬ ПО
